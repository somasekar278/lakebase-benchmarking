...
if RUN_ALL_MODES:
    MODES_TO_RUN = ["serial", "binpacked", "binpacked_parallel"]
else:
...
print("="*80)
print("⚙️  MULTI-ENTITY ZIPFIAN BENCHMARK V3 CONFIGURATION")
print("="*80)
...
def persist_results(conn, hot_pct, results):
    ...
    with conn.cursor() as cur:
        ...
        """, (
            ...
            f"Multi-entity Zipfian benchmark (mode: {results['fetch_mode']}, cache: {results['cache_state']})"
        ))
    conn.commit()
...
for mode_idx, current_mode in enumerate(MODES_TO_RUN):
    ...
    print(f"   Queries/request: {30 if current_mode == 'serial' else 3}")
    print()
    ...
    if current_mode == "binpacked_parallel":
        ...
        pool = ConnectionPool(
            conninfo=psycopg.conninfo.make_conninfo(**LAKEBASE_CONFIG),
            min_size=10,
            max_size=30,  # ✅ FIX: Larger pool (parallelism × entities × safety margin)
            timeout=10.0,  # ✅ FIX: Fail fast on pool exhaustion (was 30s)
            max_idle=300,  # Recycle idle connections after 5 min
            max_lifetime=1800,  # Force recycle after 30 min
            check=ConnectionPool.check_connection  # ✅ CRITICAL: Health check before handing out
        )
